...
In the case of `LedgerHQ-app-plugin-stakekit` we want to test the code that is responsible for handling the contract data.
...
docker build -t LedgerHQ-app-plugin-stakekit --file .clusterfuzzlite/Dockerfile .
```

### Compilation

```shell
docker run --rm --privileged -e FUZZING_LANGUAGE=c -v "$(realpath .)/fuzzing/out:/out" -ti LedgerHQ-app-plugin-stakekit
```
